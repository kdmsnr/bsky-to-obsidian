...
Blueskyのページで「設定 > アカウント > 私のデータをエクスポートする > CARファイルをダウンロード」を選択し、ファイルをスクリプトと同じディレクトリに保存します。デフォルトでは `repo.car` というファイル名を使います（別の名前で保存する場合は、`extract.car_path` を変更してください）。
