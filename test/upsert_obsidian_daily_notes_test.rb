...
assert_equal(
  "TODO:\n- 講演スライドの校正\n  - 講義スライドの作成\n- [x] 年末調整",
  ObsidianNotes.normalize_post_text("TODO:\r\n- [ ] 講演スライドの校正\r\n  - [ ] 講義スライドの作成\r\n- [x] 年末調整\r\n"),
  "unchecked task markers are rendered as plain bullets"
)

post = ObsidianNotes::Post.new(
  id: "app.bsky.feed.post/example",
  url: "https://bsky.app/profile/did:plc:self/post/example",
...
assert_equal(
  ...
  ObsidianNotes.render([post], :bsky, "UTC"),
...
].map { |timestamp| ObsidianNotes::Post.new(created_at: Time.iso8601(timestamp), text: timestamp) }

assert_equal(
  dated_posts,
  ObsidianNotes.filter_posts_by_days(dated_posts, nil, "Asia/Tokyo", now: now),
  "omitting days preserves the full input"
)
assert_equal(
  dated_posts[1..4],
  ObsidianNotes.filter_posts_by_days(dated_posts, 7, "Asia/Tokyo", now: now),
  "seven days includes whole local dates and excludes tomorrow"
)
assert_equal(
  dated_posts[3..4],
  ObsidianNotes.filter_posts_by_days(dated_posts, 1, "Asia/Tokyo", now: now),
  "one day includes only today in the configured timezone"
)
assert_equal(
  dated_posts[2..3],
  ObsidianNotes.filter_posts_by_days(dated_posts, 1, "UTC", now: now),
